...
!!! info "Free tier access"
    All users have full access to the historical archive with no date restrictions. Use the `/range` endpoint to check what coverage is available for an object before querying.
...
curl -H "X-Api-Key: YOUR_KEY" \
  "{{BASE_URL}}/api/v1/gp/25544/history?start=2024-01-01T00:00:00Z&end=2024-02-01T00:00:00Z"
```
...
```bash
curl -H "X-Api-Key: YOUR_KEY" \
  "{{BASE_URL}}/api/v1/gp/25544/nearest?epoch=2024-06-15T12:00:00Z"
```
...
```bash
curl -H "X-Api-Key: YOUR_KEY" \
  "{{BASE_URL}}/api/v1/gp/25544/adjacent?epoch=2024-06-15T12:00:00Z"
```
...
## `GET /gp/{norad_id}/range`

Returns the epoch range and total element set count for an object. Use this to check what historical coverage is available before querying.
...
```bash
curl -H "X-Api-Key: YOUR_KEY" \
  "{{BASE_URL}}/api/v1/gp/25544/range"
```
...
```json
{
  ...
  "total_elsets": 31380,
  "year_files": 22,
  "earliest_epoch": "2001-04-30T20:13:16.671648Z",
  "latest_epoch": "2026-03-22T03:54:40.385952Z"
}
```
...
| `total_elsets` | integer | Total number of historical element sets |
| `year_files` | integer | Number of year-partitioned data files |
| `earliest_epoch` | datetime | Oldest element set epoch |
| `latest_epoch` | datetime | Most recent element set epoch |
