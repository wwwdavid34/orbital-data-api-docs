...
```bash
curl -H "X-Api-Key: YOUR_KEY" \
  "{{BASE_URL}}/api/v1/catalog?owner=PRC&object_type=PAYLOAD&per_page=2"
```
...
```json
{
  ...
  "pagination": {
    ...
    "per_page": 2,
    "total_pages": 156,
    "next": "{{BASE_URL}}/api/v1/catalog?owner=PRC&object_type=PAYLOAD&per_page=2&page=2",
    "prev": null
  }
}
```
