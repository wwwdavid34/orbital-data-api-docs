...
```bash
curl -H "X-Api-Key: YOUR_KEY" \
  "{{BASE_URL}}/api/v1/gp?group=starlink&per_page=2"
```
...
```json
{
  ...
  "pagination": {
    ...
    "per_page": 2,
    "total_pages": 3211,
    "next": "{{BASE_URL}}/api/v1/gp?group=starlink&per_page=2&page=2",
    "prev": null
  }
}
```
...
```bash
curl -H "X-Api-Key: YOUR_KEY" \
  "{{BASE_URL}}/api/v1/gp?search=hubble"
```
...
```bash
curl -H "X-Api-Key: YOUR_KEY" \
  "{{BASE_URL}}/api/v1/gp?norad_ids=25544,43873,44506"
```
